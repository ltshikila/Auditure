...
export default function SeeAllScreen() {
    ...
    const renderEpisodeItem = ({ item }: { item: EpisodeFeedItem }) => (
        ...
        >
            ...
            {item.progressPercent !== undefined && item.progressPercent > 0 && (
                <View className="absolute bottom-0 left-3 right-3 h-1 bg-gray-200 dark:bg-brand-dark-border rounded-full overflow-hidden">
                    <View
    ...
    const renderBookItem = ({ item }: { item: BookFeedItem }) => (
        ...
        >
            {/* Cover */}
            <View className="w-[70px] h-[100px] rounded-lg overflow-hidden bg-brand-input dark:bg-brand-dark-input mr-3">
    ...
    const renderPodcasterItem = ({ item }: { item: PodcasterFeedItem }) => (
        ...
        >
            {/* Profile Picture */}
            <View className="w-[70px] h-[70px] rounded-full overflow-hidden bg-brand-input dark:bg-brand-dark-input mr-3">
            ...
            {/* Info */}
            <View className="flex-1 justify-center">
                <Text className="font-inter-medium text-[#1A1C1E] dark:text-brand-dark-text text-base" numberOfLines={1}>
                    {item.name}
                </Text>
                {item.bio && (
                    <Text className="font-inter text-[#858585] dark:text-brand-dark-text-secondary text-sm mt-1" numberOfLines={2}>
                        {item.bio}
                    </Text>
                )}
                <View className="flex-row items-center mt-2">
    ...
    const renderEmptyState = () => (
        <View className="flex-1 items-center justify-center py-20">
            <Text className="font-inter-medium text-lg text-[#858585] dark:text-brand-dark-text-secondary">No items found</Text>
        </View>
    );
